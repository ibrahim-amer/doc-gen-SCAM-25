/**
* Calculates traffic for each link based on routing matrix and traffic demands.
*
* @param ROUTING_MATRIX A matrix representing routing paths and their associated traffic.
* @param TRAFFIC_VECTOR A vector containing traffic demands for each route.
* @param links_list A reference to a list of link structures to store computed traffic.
* @return A vector of link_struct containing computed traffic for each link.
*/
vector<link_struct> forward_problem(vector< vector<double> > ROUTING_MATRIX,vector<demand_struct> TRAFFIC_VECTOR,vector<link_struct> &links_list)
{
